...
async def checkSnail(message: discord.Message):
    ...
    for i in range(len(queue)):
        if (queue[i][0] == linkid):
            ...
            responses = [
                f'Sorry, {message.author.name}, but that is the biggest snail I\'ve ever seen! It was first posted by {sniper.display_name} {" ".join(temptime) or "0 seconds"} ago and has since been posted {disutils.numeral_adverb(queue[i][3]- 1)}.',
                f'Lol {message.author.name}, you\'re such a snail, {sniper.display_name} already posted that {" ".join(temptime) or "0 seconds"} ago',
                ...
                f'Hey, {message.author.name}, if you were to scroll up instead of piping your twitterfeed straight into discord you would see the same post just {" ".join(temptime) or "0 seconds"} earlier',
                f'I regret to inform you, {message.author.name}, but it has been {" ".join(temptime) or "0 seconds"} since this was first posted.'
            ]
            await disutils.messageCarousel(message, responses)
