...
class ClientTest(unittest.TestCase):
    ...
    def test_parse_movie_basic(self):
        ...
        message_1 = Message(
            content = "Can't wait to watch DUNE (2021) on IMAX"
        )
    ...
    def test_parse_movie_spaced(self):
        ...
        message_1 = Message(
            content = "Can't wait to watch POOR THINGS (2023)"
        )
        res = parse_movie(message_1)
    ...
    def test_parse_movie_colon(self):
        ...
        message_1 = Message(
            content = "Can't wait to watch DUNE: PART TWO (2023)"
        )
        res = parse_movie(message_1)
    ...
    def test_parse_movie_apo(self):
        ...
        message_1 = Message(
            content = "Can't wait to watch THE TEACHERS' LOUNGE (2024)"
        )
        res = parse_movie(message_1)
    ...
    def test_create_letterboxd_string(self):
        ...
        assert res[0] == "https://letterboxd.com/film/dune-part-two-2024/reviews/by/activity/"
        assert res[1] == "https://letterboxd.com/film/dune-part-two/reviews/by/activity/"
    
    def test_cinema_check(self):

        logger = logging.getLogger()
        client = Maggus(intents=discord.Intents.all(), log=logger, in_test=True, test_loop=TaskConsumingLoop())
        message_1 = Message(
            content="Can't wait to walk AROUND the BEACH (I am quite old)"
        )
    ...
    def test_cinema_check(self):
        ...
        client = Maggus(intents=discord.Intents.all(), log=logger, in_test=True, test_loop=TaskConsumingLoop())
        message_1 = Message(
            content="Can't wait to DUNE (2021) on IMAX"
        )
        client.cinephile(message_1)
        client.loop.run_tasks()

        assert message_1.channel.test_message_sent.embed.title == "DUNE 2021"
        assert message_1.channel.test_message_sent.embed.url == "https://letterboxd.com/film/dune-2021"
        assert message_1.channel.test_message_sent.embed.description == "This is parsed text from letterboxd"
    ...
    def test_cinema_check(self):
        ...
        assert message_1.channel.test_message_sent.embed.url == "https://letterboxd.com/film/dune"
        assert message_1.channel.test_message_sent.embed.description == "This is parsed text from letterboxd"
